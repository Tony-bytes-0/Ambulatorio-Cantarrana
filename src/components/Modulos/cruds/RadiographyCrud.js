...
import { setRadiographyList } from "../../../indexModles/features/radiography/radiographyList";
...
function BasicTable() { const [selected, setSelected] = useState(emptyObject)	
    ...
    function openModal(rowId){//al abrir la modal, se llenan los datos del usuario seleccionado: "selected"
        ...
        const found = radiographyList.find( x =>  x.id === rowId)
    ...
    //CRUD
    const radiographyList = useSelector(state => state.radiographyList)
    
    
    function deleteUser(id){
        axios.delete('http://localhost:300/timg/' + id)
            .then(response => {
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>{/* aqui defino la cantidad de filas! */}
            ...
            <TableCell align="center"><b>Tipo de Radiografia</b></TableCell>
...
function CreateAllergie (props){
    ...
    function postData(){//extends
        if(data === ''){alert('inserte algun dato')}
            ...
            axios.post('http://localhost:300/timg/', {nombre:data})
            .then((response) =>{
    ...
    function updateData(){
        if(data === ''){alert('inserte algun dato')}
            ...
            axios.put('http://localhost:300/timg/' + props.id, {tipo:data})
            .then((response) =>{
    ...
    return <><Grid container direction="column" alignItems="center" justifyContent="center">
        <TextField variant="outlined" label="Tipo de Radiografia" value={data} onChange={handleData} />
        <RenderedButton update={props.toggleUpdate} />
    </Grid></>
}

export default function RadiographyCrud(){//Main
  const [loading, setLoading] = useState(false)
    const dispatch = useDispatch()

    async function fetchLabTest() {
        setLoading(true)
        axios.get('http://localhost:300/timg/')//Debo hacer esto dinamico
            .then(response => {
    ...
    return<>
        <Grid container style={{"padding":"2%"}} className='fadeIn'>
          <Grid item xs = {12}><div className="centrate separator basicBorders tittle"><h4><b>Gestionar Radiografias</b></h4></div> </Grid>

            <CreateAllergie/>
            <Grid item xs={12} >
                <div className="centrate" style={{"position":"relative","top":"10%"}} >
                    <Button id='getBtn' variant='contained' fullWidth onClick={() => {fetchLabTest()}}>Mostrar Radiografias</Button>
                </div>    
            </Grid>
