...
import { setDisease } from "../../../indexModles/features/Symptoms/diseases";
...
function BasicTable() { const [selected, setSelected] = useState(emptyObject)	
    ...
    function openModal(rowId){//al abrir la modal, se llenan los datos del usuario seleccionado: "selected"
        ...
        const found = diseases.find( x =>  x.id === rowId)
    ...
    function deleteUser(id){
        axios.delete('http://localhost:300/enfermedad/' + id)
            .then(response => {
      ...
      <Table sx={{ minWidth: 650 }} aria-label="simple table">
        <TableHead>
          <TableRow>{/* aqui defino la cantidad de filas! */}
            ...
            <TableCell align="center"><b>Enfermedades</b></TableCell>
...
function CreateAllergie (props){
    ...
    function postData(){//extends
        if(data === ''){alert('inserte algun dato')}
            ...
            axios.post('http://localhost:300/enfermedad/', {nombre:data})
            .then((response) =>{
    ...
    function updateData(){
        if(data === ''){alert('inserte algun dato')}
            ...
            axios.put('http://localhost:300/enfermedad/' + props.id, {nombre:data})
            .then((response) =>{
    ...
    return <><Grid container direction="column" alignItems="center" justifyContent="center">
        <TextField variant="outlined" label="Nueva Enfermedad" value={data} onChange={handleData} />
        <RenderedButton update={props.toggleUpdate} />
    </Grid></>
}

export default function DiseasesCrud(){//Main
  const [loading, setLoading] = useState(false)
    const dispatch = useDispatch()

    async function fetchLabTest() {
        setLoading(true)
        axios.get('http://localhost:300/enfermedad/')//Debo hacer esto dinamico
            .then(response => {
    ...
    return<>
        <Grid container style={{"padding":"2%"}} className='fadeIn'>
          <Grid item xs = {12}><div className="centrate separator basicBorders tittle"><h4><b>Gestionar Enfermedades</b></h4></div> </Grid>

            <CreateAllergie/>
            <Grid item xs={12} >
                <div className="centrate" style={{"position":"relative","top":"10%"}} >
                    <Button id='getBtn' variant='contained' fullWidth onClick={() => {fetchLabTest()}}>Mostrar Enfermedades</Button>
                </div>    
            </Grid>
